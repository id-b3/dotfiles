    ...
    vim.system({ "ruff", "check", "--output-format=json", "." }, { text = true }, function(out)
        if out.code ~= 0 and out.code ~= 1 then
            vim.schedule(function()
                vim.notify(
                    "Ruff failed to execute (exit code " .. tostring(out.code) .. "):\n" .. (out.stderr or "Command not found."),
        ...
        vim.schedule(function()
            ...
            -- DEBUG: Try loading trouble
            local has_trouble, trouble_or_err = pcall(require, "trouble")
            if not has_trouble then
                vim.notify(
                    "DEBUG: Trouble failed to load!\nError: " .. tostring(trouble_or_err), 
                    vim.log.levels.ERROR, 
                    { title = "Trouble Debug" }
                )
            else
                print("DEBUG: trouble.nvim loaded successfully.")
            end
            ...
            if #qf_items > 0 then
                ...
                if has_trouble then
                    ...
                    if not success then
                        vim.notify(
                            "DEBUG: Trouble loaded, but failed to open!\nError: " .. tostring(err), 
                            vim.log.levels.ERROR, 
                            { title = "Trouble Debug" }
                        )
                    else
                ...
                if not opened_with_trouble then
                    print("DEBUG: Falling back to native quickfix window.")
                    vim.cmd("copen")
