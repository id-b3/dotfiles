...
- **`<Tab>` / `<S-Tab>` behavior:** the custom handlers prioritize LuaSnip jumps when actively inside a snippet; otherwise they navigate the completion menu when visible, then fall back.
- **Docs popup behavior:** completion item documentation auto-shows with a 500ms delay (`completion.documentation.auto_show = true`).
...
- **Which Copilot spec loads:** `init.lua` imports `dudzie.text_editing`, so `lua/dudzie/text_editing/copilot.lua` is the active Copilot config. The previous duplicate at `lua/dudzie/plugins/copilot.lua` has been removed.
